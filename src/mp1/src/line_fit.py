...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		cv2.rectangle(out_img, (window_leftx_low, window_y_low), 
					(window_leftx_high, window_y_high), (0,255, 0))
		cv2.rectangle(out_img, (window_rightx_low, window_y_low), 
					(window_rightx_high, window_y_high), (0,255, 0))
		# cv2.imshow('image', out_img)
		# cv2.waitKey(0)
		####
		# Identify the nonzero pixels in x and y within the window
		## TODO
		left_inrange_inds = ((nonzeroy > window_y_low) & (nonzeroy < window_y_high)
			   & (nonzerox > window_leftx_low) & (nonzerox < window_leftx_high))
		left_inds = np.where(left_inrange_inds)[0]
		right_inrange_inds = ((nonzeroy > window_y_low) & (nonzeroy < window_y_high)
			   & (nonzerox > window_rightx_low) & (nonzerox < window_rightx_high))
