...
class LidarProcessing:
    def __init__(
        ...
        self.pointCloudSub = rospy.Subscriber(
            "/velodyne_points", PointCloud2, self.__pointCloudHandler, queue_size=10
        )
    ...
    def __pointCloudHandler(self, data):
        ...
        """
        gen = point_cloud2.readgen = point_cloud2.read_points(
            cloud=data, field_names=("x", "y", "z", "ring")
        )
    ...
    def construct_birds_eye_view(self, data):
        ...
        x_filter = np.logical_and(
            (x_points >= self.fwd_range[0]), (x_points <= self.fwd_range[1])
        )
        y_filter = np.logical_and(
            (y_points >= -self.side_range[1]), (y_points <= -self.side_range[0])
        )
        ...
        pixel_vals = np.clip(
            a=z_points, a_min=self.height_range[0], a_max=self.height_range[1]
        )

        pixel_vals = scale_to_255(
            pixel_vals, min_val=self.height_range[0], max_val=self.height_range[1]
        )
        ...
        # print("fl", x_points[indices])

        # Getting sensor reading for front right
        filter_front_right = np.logical_and(
            (x_points + y_points > -0.1), (x_points + y_points < 0.1)
        )
        ...
        # print("fr", x_points[indices])
        # print(x_points[indices])

        # Getting sensor reading for rear left
        filter_rear_left = np.logical_and(
            (x_points + y_points > -0.1), (x_points + y_points < 0.1)
        )
        ...
        # print("rl", x_points[indices])

        # Getting sensor reading for rear right
        filter_rear_right = np.logical_and(
            (x_points - y_points > -0.1), (x_points - y_points < 0.1)
        )
