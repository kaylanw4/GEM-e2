...
class PID(object):
    def __init__(self, kp, ki, kd, wg=None):  # 0.5, 0.0, 0.1, 20
        self.iterm = 0
        self.last_t = None
        self.last_e = 0
        self.kp = kp
...
class PurePursuit(object):
    def __init__(self):
        ...
        self.gnss_sub = rospy.Subscriber(
            "/novatel/inspva", Inspva, self.inspva_callback
        )
        self.lat = 0.0
        self.lon = 0.0
        self.heading = 0.0
        ...
        self.enable_sub = rospy.Subscriber(
            "/pacmod/as_tx/enable", Bool, self.enable_callback
        )
        ...
        self.max_accel = 0.42  # % of acceleration
        self.pid_speed = PID(0.5, 0.0, 0.05, wg=20)
        ...
        self.steer_cmd.angular_position = (
            0.0  # radians, -: clockwise, +: counter-clockwise
        )
    ...
    def start_pp(self):
        while not rospy.is_shutdown():
            ...
            self.desired_speed, angle, acceleration = self.get_data_from_state()
            ...
            output_accel = self.pid_speed.get_control(
                current_time, self.desired_speed - filt_vel
            )
            ...
            #     output_accel = 0.2
            # else:
            #     output_accel = acceleration

            if output_accel < 0:
                self.brake_cmd.f64_cmd = 0.2
                self.brake_pub.publish(self.brake_cmd)
            else:
                if output_accel < 0.3:
                    output_accel = 0.3
