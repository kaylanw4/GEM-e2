...
class lanenet_detector:
    ...
    def combinedBinaryImage(self, img):
        ...
        ColorOutput = self.color_thresh(img, thresh=(220, 255))
    ...
    def guess_valid_lane(self, ret_l, ret_r, binary_warped):
        ...
        # print(midpoint, left_fitx_low[-1])
        if left_fitx_low[-1] < midpoint:
            left_start_left = True
        else: 
            left_start_left = False
        if right_fitx_low[-1] > midpoint:
            right_start_right = True
        else: 
            right_start_right = False
        if left_start_left and not right_start_right:
            return "Left"
        if not left_start_left and right_start_right:
            return "Right"
        if left_start_left and right_start_right:
            return "Both"
        ...
        if not self.hist:
            ...
            if detected_left and detected_right:
                ...
                if np.sum(left_lane_idx & right_lane_idx) > 30:
                    ...
                    # Two lane overlap
                    print("hit")
                    valid_lane = self.guess_valid_lane(ret_l, ret_r, img_birdeye)
                    if self.state == "LEFT":
                        ret_r = None
